...
public class SnippetQueryController {
    ...
    @GetMapping("/filter")
    public ResponseEntity<Page<SnippetPreview>> findSnips(@RequestBody SearchCriteria filterDto, @PageableDefault(size = 20, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable, @AuthenticationPrincipal AppUserPrincipal principal, Sort sort){
